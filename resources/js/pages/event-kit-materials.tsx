            ...
            <PublicLayout navActive="/event">
                <section className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
                    ...
                    <div className="grid gap-4 md:grid-cols-[1fr_auto] md:items-start">
                        ...
                        <Card className="w-full border-slate-200/70 bg-white/70 px-4 py-3 text-xs text-slate-600 shadow-sm dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-300 md:w-[320px]">
                            <div className="truncate font-semibold text-slate-900 dark:text-slate-100">
                                {participant.name}
                            </div>
                            <div className="truncate">{participant.display_id}</div>
                            <div className="truncate text-slate-500 dark:text-slate-400">{participant.email}</div>
                        </Card>
                    </div>

                    {/* Event selector */}
                    {checked_in_programmes.length ? (
                        <Card className="mt-5 border-slate-200/70 bg-white/70 p-4 dark:border-slate-800 dark:bg-slate-950/40 sm:p-5">
                            <div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
                                ...
                                <Popover open={open} onOpenChange={setOpen}>
                                    ...
                                    >
                                        <Command>
                                            <CommandInput placeholder="Search events..." className="h-10" />
                    ...
                    <div className="mt-5 grid gap-4 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
                        ...
                        <Card className="border-slate-200/70 bg-white/70 p-4 dark:border-slate-800 dark:bg-slate-950/40 sm:p-6">
                            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div className="min-w-0">
                                    <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100 sm:text-lg">
                                        <span className="block break-words">{programme.title}</span>
                                    </h2>
                                    ...
                                    <div className="mt-3 flex flex-col gap-2 text-xs text-slate-600 dark:text-slate-300">
                                        ...
                                        {programme.location ? (
                                            <span className="inline-flex items-center gap-2">
                                                ...
                                                <span className="min-w-0 break-words">{programme.location}</span>
                                            </span>
                                        ) : null}
                                    </div>
                                </div>
                            ...
                            <div className="mt-5 border-t border-slate-200/60 pt-4 dark:border-slate-800 sm:mt-6 sm:pt-5">
                        ...
                        <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                        <div className="flex min-w-0 items-center gap-2">
                                            ...
                                            <span className="min-w-0 break-words font-semibold">
                                                Print certificates (appearance & participation)
                                            </span>
                                        </div>
                            ...
                            <Card className="border-slate-200/70 bg-white/70 p-4 dark:border-slate-800 dark:bg-slate-950/40 sm:p-5">
                                <div className="flex flex-col gap-2 text-xs text-slate-600 dark:text-slate-300 sm:flex-row sm:items-center sm:justify-between">
                                    ...
                                    <form onSubmit={(event) => event.preventDefault()} className="w-full sm:w-auto">
                                        <Button
                                            type="button"
                                            variant="outline"
                                            size="sm"
                                            className="h-9 w-full rounded-xl sm:h-8 sm:w-auto"
                                            onClick={() =>
                                                resetForm.post('/event-kit/reset', {
                                                    preserveScroll: true,
                                                    onSuccess: () => toast.success('Session reset.'),
                                                    onError: () => toast.error('Unable to reset.'),
                                                })
                                            }
                                        >
